//DESAFIO ENTREGABLE 4 - ROBLEDO, BELEN
...
function comprarProductos(listaProductos){
    let carrito = retrieveCarrito(listaProductos);
...
//-----LOCAL-STORAGE----------------------------------------------------------------------------------------
function retrieveCarrito(listaProductos){
    let listaJSON = JSON.parse(localStorage.getItem("productosCarrito"));
    let carrito = new Carrito();
    if(listaJSON!=null){
        for(const objeto of listaJSON){
            let index = listaProductos.map(e => e.id).indexOf(objeto.id);
            (index !==-1) && (listaProductos[index].setCantidad(objeto.cantidad) + carrito.productos.push(listaProductos[index]));
        }
    }
    return carrito;
}
